...
flowchart TB
  ...
  class DRV weak
  linkStyle 6 stroke:#d9480f,stroke-width:3px
```

Выделенная связь **ТС ПИоТ → gRPC → объект драйвера → ККТ** и есть слабое
звено. Если «Не отключаться от ККТ» выключено, цепочка отказа такая:
...
момент авторизации совпал с открытым соединением, — а примерно через час
проверки незаметно переходят на ЛМ ЧЗ.

Чтобы канал держался, на стороне обработки нужны три условия:
