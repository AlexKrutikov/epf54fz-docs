...
Этот раздел касается **и Атол, и Штрих**: механика каналов у них разная, а
требование к размещению — одно.
...
У касс **Штрих** канал устроен иначе, но с тем же следствием: при включённом
флаге «Включить передачу информации в драйвер ТС ПИоТ (ЕСМ)» объект драйвера
сам подгружает модуль ТС ПИоТ, тот поднимает свой gRPC-сервис и получает
уведомление о подключённой кассе с её заводским номером. Живёт этот модуль
внутри процесса, создавшего объект драйвера, — то есть внутри сеанса 1С.
...
flowchart TB
  subgraph PC["Один компьютер"]
    ...
    DRV["Объект драйвера<br/>+ модуль связи с ТС ПИоТ"]
    ESM["ТС ПИоТ (ЕСМ)"]
  end
  ...
  DRV <-->|"USB / COM / TCP"| KKT
  ESM ==>|"через объект драйвера<br/>на своём ПК"| DRV
...
flowchart TB
  ...
  subgraph PC2["Сервер с ТС ПИоТ"]
    ESM["ТС ПИоТ (ЕСМ)"]
    NODRV["Объекта драйвера нет<br/>связи с ККТ нет"]
  end
  KKT["ККТ"]
  GIS[("ГИС МТ")]
  ...
  S1C ==>|"создаёт объект"| DRV
  DRV <-->|"USB / COM / TCP"| KKT
  ESM -.->|"ищет объект драйвера у себя"| NODRV
